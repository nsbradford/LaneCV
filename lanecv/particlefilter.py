...
class ParticleFilterModel():
    """ Particle filter for a single lane model. """

    LEARNING_RATE = 1e-3 # proportional to how quickly particles move to follow evidence
    VISUALIZATION_SIZE = 500 # square visualization, in pixels

    def __init__(self, particle_cls, n=1000):
        VAR_SCALING = 100
        self.particle_cls = particle_cls # class of model; LineModel
        self.VAR_OFFSET = particle_cls.OFFSET_RANGE / VAR_SCALING # noise to apply
        self.VAR_ORIENTATION = particle_cls.ORIENTATION_RANGE / VAR_SCALING # noise to apply
        self.state_size = 2 # number of elements in the state
        self.n = n # number of particles
        self.particles = self._init_particles()
        self.weights = self._init_weights()
        self.state_matrix = self._init_state()
        self.state = None # current state; instance of the particle_cls class
    ...
    def updateState(self, model_measurement):
        """ Perform a complete update step. This is the ONLY public method!
            Args:
    ...
    def updateStateNoEvidence(self):
        """ Apply state update with no evidence. 
            TODO should apply some noise to reflect growing uncertainty
        """
        return self.state
